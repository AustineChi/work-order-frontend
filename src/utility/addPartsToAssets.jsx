...
const AddPartsToAssets = ({handleChange, updateAsset, parts, closeAddPartsModal, addPartsModal, assetDetails}) => {
 console.log("assetDetails.parts", assetDetails)
  const _parts = parts.length ? (
    parts.map((data, key) => {
      return (
        ...
        type="checkbox" 
        name="parts" 
        checked={assetDetails.parts.indexOf(data.partName) !== -1}
        value={data.partName} 
        className="m-all" />
           {data.partName}
        </div>
